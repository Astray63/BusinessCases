...
import com.electriccharge.app.dto.DashboardStatsDto.ClientStats;
import com.electriccharge.app.dto.DashboardStatsDto.OwnerStats;
...
public class DashboardServiceImpl implements DashboardService {
    ...
    private ClientStats calculateClientStats(Long userId) {
        ...
        stats.setReservationsEnCours(reservationsByStatus.getOrDefault(Reservation.EtatReservation.ACTIVE, 0L).intValue());
        stats.setReservationsTerminees(reservationsByStatus.getOrDefault(Reservation.EtatReservation.TERMINEE, 0L).intValue());
        stats.setReservationsAnnulees(reservationsByStatus.getOrDefault(Reservation.EtatReservation.ANNULEE, 0L).intValue());
        ...
        nextReservation.ifPresent(reservation -> {
            try {
                ReservationDto nextResDto = reservationService.getById(reservation.getNumeroReservation());
    ...
    private OwnerStats calculateOwnerStats(Long userId, List<ChargingStation> stations) {
        ...
        stats.setBornesDisponibles(stationsByStatus.getOrDefault(ChargingStation.Etat.DISPONIBLE, 0L).intValue());
        stats.setBornesOccupees(stationsByStatus.getOrDefault(ChargingStation.Etat.OCCUPEE, 0L).intValue());
        stats.setBornesMaintenance(stationsByStatus.getOrDefault(ChargingStation.Etat.EN_MAINTENANCE, 0L).intValue());
        ...
        Map<Long, Long> reservationsByStation = allReservations.stream()
                .collect(Collectors.groupingBy(r -> r.getChargingStation().getIdBorne(), Collectors.counting()));
    ...
    private BorneDto convertChargingStationDtoToBorneDto(ChargingStationDto csDto) {
        ...
        borneDto.setConnectorType(csDto.getConnectorType());
