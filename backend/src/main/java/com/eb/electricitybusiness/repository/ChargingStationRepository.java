...
public interface ChargingStationRepository extends JpaRepository<ChargingStation, Long> {
    ...
    @Query("SELECT cs FROM ChargingStation cs LEFT JOIN FETCH cs.medias JOIN cs.lieux l WHERE l.idLieu = :idLieu")
    List<ChargingStation> findByLieuxId(@Param("idLieu") Long idLieu);

    @Query(value = "SELECT * FROM charging_stations cs " +
